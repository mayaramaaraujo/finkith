...
| 4 | Invite-by-email was half-built: `group_members.invited_email`, `status = 'invited'`, an RLS insert policy existing only for it, `resend.ts`, the `resend` dependency, and unreachable "Invited" branches in the members list. | Removed. `0011_drop_email_invites.sql` drops the column, the dead status value and the insert policy — nothing replaces the policy, since both membership paths are security-definer RPCs that bypass RLS. Invites are link-only. |
...
§5.1 records where the deployed schema diverged from the migrations, and how to confirm it no longer does.

### 5.1 The deployed schema, and how it got here

Two objects existed on the deployed database without ever having a migration — `savings_entries` and `groups.savings_initial_balance`, created in the Supabase dashboard while prototyping a savings feature that was later dropped. No client code ever read or wrote them. `0012_drop_savings.sql` removes both; because a database built from these migrations alone never had them, that migration guards its drops with `if exists`.

Separately, `0011_drop_email_invites.sql` was once recorded as applied while its effects were absent. It has since been repaired and genuinely applied — `group_members.invited_email` now correctly answers `42703 column does not exist`.

**There is no savings feature in this product** (§10). If a `savings_*` object ever reappears in generated types, it is drift, not a requirement — check `supabase/migrations/` before building anything against it.

Verify the schema matches this document before starting:

```
supabase migration list                 # local and remote columns should agree
supabase gen types typescript --linked  # should match src/shared/lib/supabase/database.types.ts
```
...
| `status` | text | CHECK `= 'active'` — the only value left after `0011`. Kept because the RLS helpers and every membership query filter on it |
| `created_at` | timestamptz | |
...
1. Confirm the deployed schema matches this document using the two commands in §5.1, before generating any types.
2. `0013_native_push_tokens.sql` — see §9. (`0011` and `0012` are taken.)
...
1. **Migration `0013_native_push_tokens.sql`:**
   ```sql
...
- Savings tracking. A `savings_entries` table and a `groups.savings_initial_balance` column briefly existed on the deployed database; the feature was dropped and `0012_drop_savings.sql` removes both.
